...
/*
 * After incrementing used_count, check whether the range just became fully
 * packed.  If so, record filled_at and set the packed flag so the recycle
 * countdown can begin.
 */
...
void dhcp_unifpool_recycle(dhcp_bmpool_uni_t *pool, uint32_t current_time)
{
    for (uint8_t i = 0; i < pool->num_ranges; ++i) {
        dhcp_unifrange_t *r = &pool->ranges[i];
        /* Only packed ranges have a running recycle timer */
...
uint32_t dhcp_unifpool_peek(dhcp_bmpool_uni_t *pool, uint32_t current_time)
{
    ...
    /* Active, non-packed range with a free slot */
    for (uint8_t i = 0; i < pool->num_ranges; ++i) {
        dhcp_unifrange_t *r = &pool->ranges[i];
        if (!r->active || r->packed) continue;
        int slot = bm_find_free(r->bitmap, r->range_size);
                                 ...
                                 uint32_t current_time)
{
    ...
    if (r->packed) return 0u;                          /* range is full */
    if (dhcp_bm_used(r->bitmap, bit_idx)) return 0u;  /* IP already taken */
...
uint32_t dhcp_unifpool_lease_time(const dhcp_bmpool_uni_t *pool, uint32_t ip)
{
    ...
    /* All clients in a range receive the same full duration */
