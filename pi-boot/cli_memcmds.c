...
#define PRINT_HEX(x) uart_put_hex32(PTR(x), 1)
...
void cmd_hexdump(const char *args) {
    ...
    for (uint32_t i = 0; i < len; i += 16) {
        uart_put_hex32((uint32_t)(ptr + i), 0);
        uart_puts(": ");
...
void cmd_info(const char *args UNUSED) {
    ...
    uint32_t _stack_bottom = (uint32_t)&_stack_top - TOTAL_STACK;
...
void cmd_usage(const char *args UNUSED) {
    ...
    uint32_t sp, st_used, st_free;
    asm volatile("mov %0, sp": "=r"(sp));
    st_used = (uint32_t)&_stack_top - sp;
    st_free = TOTAL_STACK - st_used;

    uint32_t static_size, reserved_ram, free_ram;
    reserved_ram = (uint32_t) &_stack_top - (uint32_t) &__text_start;
    free_ram = TOTAL_RAM - reserved_ram; 
    static_size = (uint32_t) &__bss_end - (uint32_t)&__text_start; 
