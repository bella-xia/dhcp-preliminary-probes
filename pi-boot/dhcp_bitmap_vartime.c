...
static void varrange_create_inactive(dhcp_bmpool_var_t *pool, uint8_t i)
{
    dhcp_varrange_t *r = &pool->ranges[i];
...
void dhcp_varpool_recycle(dhcp_bmpool_var_t *pool, uint32_t current_time)
{
    for (uint8_t i = 0; i < pool->num_ranges; ++i) {
        ...
        if ((current_time - r->activated_at) >= r->lease_duration) {
            kmemset(r->bitmap, 0, sizeof(r->bitmap));
            r->used_count = 0;
            r->active     = 0;
        }
...
{
    dhcp_varpool_recycle(pool, current_time);

    /* 1. Free slot in an already-active range */
    for (uint8_t i = 0; i < pool->num_ranges; ++i) {
        dhcp_varrange_t *r = &pool->ranges[i];
        if (!r->active || r->used_count >= r->range_size) continue;
    ...
    varrange_create_inactive(pool, pool->num_ranges);
    dhcp_varrange_t *r = &pool->ranges[pool->num_ranges];
    r->active       = 1;
    r->activated_at = current_time;
    r->used_count   = 1;
    dhcp_bm_set(r->bitmap, 0);
...
uint32_t dhcp_varpool_peek(dhcp_bmpool_var_t *pool, uint32_t current_time)
{
    ...
    /* First IP of an inactive (recycled) range */
    for (uint8_t i = 0; i < pool->num_ranges; ++i) {
        if (!pool->ranges[i].active) return pool->ranges[i].range_start;
    }

    /* Would need a fresh slice */
                                ...
                                uint32_t current_time)
{
    ...
    /* Create placeholder inactive ranges up to range_idx if needed */
    ...
    dhcp_varrange_t *r = &pool->ranges[range_idx];
    if (!r->active) {
        r->active       = 1;
        r->activated_at = current_time;
        /* bitmap already zeroed from init or recycle */
    }
    if (dhcp_bm_used(r->bitmap, bit_idx)) return 0u; /* already in use */
...
uint8_t dhcp_varpool_is_used(const dhcp_bmpool_var_t *pool, uint32_t ip)
{
    ...
    const dhcp_varrange_t *r = &pool->ranges[range_idx];
                                       ...
                                       uint32_t ip, uint32_t current_time)
{
    ...
    uint32_t elapsed = current_time - r->activated_at;
    return (elapsed < r->lease_duration) ? (r->lease_duration - elapsed) : 0u;
}
                         ...
                         uint8_t  *out_num_active)
{
    ...
    uint8_t  active = 0;
    for (uint8_t i = 0; i < pool->num_ranges; ++i) {
        const dhcp_varrange_t *r = &pool->ranges[i];
        if (!r->active) continue;
        used  += r->used_count;
        total += r->range_size;
        active++;
